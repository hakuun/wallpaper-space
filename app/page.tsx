...
export default function HomePage() {
	...
	useEffect(() => {
		...
		getImage().then((res) => setImages(res));
	}, [lastViewedPhoto, setLastViewedPhoto, setImages]);

	async function getImage() {
		const res = await api("/api/wallpaper");
		if (!res.records) return [];
		const images = await generateImages(res.records);
		return images;
	}
